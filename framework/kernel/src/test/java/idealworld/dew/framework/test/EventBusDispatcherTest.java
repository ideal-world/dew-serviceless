...
public class EventBusDispatcherTest extends DewTest {
    ...
    public void testProcess(Vertx vertx, VertxTestContext testContext) {
        ...
        EventBusDispatcher.addProcessor("", OptActionKind.CREATE, "/app/{name}/{kind}/enabled", MockProcessor4());

        var count = new CountDownLatch(3);

        Future.succeededFuture()
                .compose(resp -> EventBusDispatcher.chooseProcess("", null, null, OptActionKind.CREATE, "/app", "q=测试", new HashMap<>(), null))
                .compose(resp -> {
                    Assertions.assertEquals("/app", resp);
                    return Future.succeededFuture();
                })
                .compose(resp -> EventBusDispatcher.chooseProcess("", null, null, OptActionKind.CREATE, "/app/n1", "q=测试", new HashMap<>(), null))
                .compose(resp -> {
                    Assertions.assertEquals("/app/{name}", resp);
                    return Future.succeededFuture();
                })
                .compose(resp -> EventBusDispatcher.chooseProcess("", null, null, OptActionKind.CREATE, "/app/n1/k1", "q=测试", new HashMap<>(), null))
                .compose(resp -> {
                    Assertions.assertEquals("/app/{name}/{kind}", resp);
                    return Future.succeededFuture();
                })
                .compose(resp -> EventBusDispatcher.chooseProcess("", null, null, OptActionKind.CREATE, "/app/n1/k1/enabled", "q=测试", new HashMap<>(),
