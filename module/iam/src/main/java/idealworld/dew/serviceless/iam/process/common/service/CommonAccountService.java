...
package idealworld.dew.serviceless.iam.process.common.service;
...
public class CommonAccountService extends IAMBasicService {
    ...
    public Resp<IdentOptInfo> register(AccountRegisterReq accountRegisterReq) {
        var tenantIdR = commonProcessor.getEnabledTenantIdByAppId(accountRegisterReq.getRelAppId(), true);
        if (!tenantIdR.ok()) {
        ...
        var validRuleAndGetValidEndTimeR = commonProcessor.validRuleAndGetValidEndTime(
        ...
        var processIdentSkR = commonProcessor.processIdentSk(accountRegisterReq.getKind(),
                accountRegisterReq.getAk(),
                accountRegisterReq.getSk(),
    ...
    public Resp<IdentOptInfo> login(AccountLoginReq accountLoginReq) {
        var tenantIdR = commonProcessor.getEnabledTenantIdByAppId(accountLoginReq.getRelAppId(), false);
        if (!tenantIdR.ok()) {
            return Resp.error(tenantIdR);
        }
    ...
    protected Resp<IdentOptInfo> loginWithoutAuth(Long accountId, String openId, String ak, Long appId, Long tenantId) {
        ...
        optInfo.setRoleInfo(commonProcessor.findRoleInfo(accountId));
        optInfo.setGroupInfo(commonProcessor.findGroupInfo(accountId));
    ...
    public Resp<Void> changeIdent(AccountIdentChangeReq accountIdentChangeReq, String relOpenId, Long relAppId) {
        var tenantIdR = commonProcessor.getEnabledTenantIdByOpenId(relOpenId);
        if (!tenantIdR.ok()) {
            return Resp.error(tenantIdR);
        }
        var validRuleAndGetValidEndTimeR = commonProcessor.validRuleAndGetValidEndTime(
        ...
        var processIdentSkR = commonProcessor.processIdentSk(accountIdentChangeReq.getKind(),
                accountIdentChangeReq.getAk(),
                accountIdentChangeReq.getSk(),
                relAppId,
                tenantIdR.getBody());
