...
(
    255
) not null comment 'Key',
    v varchar
(
    3000
) not null comment 'Value',
...
(
    255
) not null comment '对象名称',
    record_id varchar
(
    255
) not null comment '记录Id',
...
(
    100
) not null comment 'Open Id',
    name varchar
(
    255
) not null comment '账号名称',
    avatar varchar
(
    1000
) not null comment '账号头像（路径）',
    parameters varchar
(
    2000
) not null comment '账号扩展信息，Json格式',
    parent_id bigint not null comment '父账号Id，不存在时为空',
    status varchar
(
    50
) not null comment '账号状态',
...
create
index IDXqoxo6ajinakf764bmxk92w7y2
...
create
index IDXi9de746blpeo08vafc3dr6ind
...
(
    255
) not null comment '绑定使用的账号认证类型名称',
...
create
index IDXd5pbls2wrseb56lie3woxi6ak
    on iam_account_bind (to_tenant_id);

create
index IDXvlkusrg2ubeq6vskcmxp08qp
...
create
index IDX3ry6uai59gurqtvuu7nqikuqe
...
(
    100
) not null comment '账号认证类型名称',
    ak varchar
(
    255
) not null comment '账号认证名称',
    sk varchar
(
    255
) not null comment '账号认证密钥',
...
create
index IDX4haa22xu0379nl99a9qvywh7c
...
(
    1000
) not null comment '应用图标（路径）',
    parameters varchar
(
    2000
) not null comment '应用扩展信息，Json格式',
    pri_key varchar
(
    1000
) not null comment '私钥',
...
create
index IDX1jjwc4sh23jovpthkvapym5ix
    on iam_app (status);
...
(
    255
) not null comment '应用认证名称（Access Key Id）',
    sk varchar
(
    1000
) not null comment '应用认证密钥（Secret Access Key）',
    valid_time bigint not null comment '应用认证有效时间',
    note varchar
(
    1000
) not null comment '应用认证用途',
...
create
index IDXi4gnko54gfoijmtxfe9p2nfl4
...
(
    20
) not null comment '关联权限主体类型名称',
    rel_subject_ids varchar
(
    10000
) not null comment '关联权限主体Ids,有多个时逗号分隔,注意必须存在最后一个逗号',
    subject_operator varchar
(
    20
) not null comment '关联权限主体运算类型名称',
    action_kind varchar
(
    100
) not null comment '操作类型名称',
    rel_resource_id bigint not null comment '关联资源Id',
    result_kind varchar
(
    100
) not null comment '操作结果名称',
    effective_time bigint not null comment '生效时间',
    exclusive tinyint
(
    1
) not null comment '是否排他',
...
(
    100
) not null comment '群组类型名称',
    code varchar
(
    255
) not null comment '群组编码',
    name varchar
(
    255
) not null comment '群组名称',
    icon varchar
(
    1000
) not null comment '群组图标（路径）',
    expose_kind varchar
(
    100
) not null comment '开放等级类型名称',
...
create
index IDXfs1qyakrurqoi32v30xxbp1ne
...
(
    1000
) not null comment '节点编码',
    bus_code varchar
(
    1000
) not null comment '业务编码',
    name varchar
(
    255
...
(
    2000
) not null comment '节点扩展信息，Json格式',
...
(
    1
) not null comment '是否是资源组',
    action varchar
(
    1000
) not null comment '触发后的操作，多用于菜单链接',
...
create
index IDX2tig999ttn9s7d67l6wifr3bw
    on iam_resource (expose_kind);

create
index IDXqtt1cl5bpsru88k4ut4x54gim
...
(
    100
) not null comment '资源类型名称',
    code varchar
(
    255
) not null comment '资源主体编码',
    uri varchar
(
    5000
) not null comment '资源主体连接URI',
    name varchar
(
    255
) not null comment '资源主体名称',
    ak varchar
(
    1000
) not null comment 'AK，部分类型支持写到URI中',
    sk varchar
(
    1000
) not null comment 'SK，部分类型支持写到URI中',
    platform_account varchar
(
    1000
) not null comment '第三方平台账号名',
    platform_project_id varchar
(
    1000
) not null comment '第三方平台项目名，如华为云的ProjectId',
...
create
index IDXei74af3xufobhvh61mujbngqt
...
create
index IDXnaxipcsq8ps9d86qvrg5a5e3k
...
(
    255
) not null comment '角色定义编码',
    name varchar
(
    255
) not null comment '角色定义名称',
    sort int not null comment '显示排序，asc',
    rel_app_id bigint not null comment '关联应用Id',
    rel_tenant_id bigint not null comment '关联租户Id',
    create_time timestamp default CURRENT_TIMESTAMP null comment '创建时间',
...
(
    1000
) not null comment '租户图标（路径）',
    parameters varchar
(
    5000
) not null comment '租户扩展信息，Json格式',
    allow_account_register tinyint
(
    1
) not null comment '是否开放账号注册',
    status varchar
(
    50
) not null comment '租户状态',
    create_time timestamp default CURRENT_TIMESTAMP null comment '创建时间',
    create_user bigint not null comment '创建者Id',
    update_time timestamp default CURRENT_TIMESTAMP null on update CURRENT_TIMESTAMP comment '最后一次修改时间',
    update_user bigint not null comment '最后一次修改者Id'
...
create
index IDX3x7h6khiqtlwf1uewxh78xfx9
...
(
    100
) not null comment '租户认证类型名称',
    valid_ak_rule varchar
(
    2000
) not null comment '认证AK校验正则规则',
    valid_ak_rule_note varchar
(
    2000
) not null comment '认证AK校验正则规则说明',
    valid_sk_rule varchar
(
    2000
) not null comment '认证SK校验正则规则',
    valid_sk_rule_note varchar
(
    2000
) not null comment '认证SK校验正则规则说明',
